...
function pro_league_scripts() {
    ...
    if (is_page($exclude_pages) || $post_type=="ligue") {
        ...
        wp_enqueue_style('custom-styles', get_stylesheet_directory_uri() . '/custom.css', [], '1.0');
        // Charger un script JS alternatif
        wp_enqueue_script(
            'custom-js',
            get_stylesheet_directory_uri() . '/custom.js',
            ['jquery'], // dépendance si tu utilises jQuery
            '1.0',
            true // charge en footer
        );
    } else {
        // Charger le style principal du thème
        wp_enqueue_style('pro-league-style', get_stylesheet_uri(), [], _S_VERSION);
...
add_action('after_setup_theme', function() {
    if (current_user_can('joueur_jpl')) {
