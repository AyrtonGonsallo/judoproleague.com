...
$sorted_result_ids=array();
	...
	$sorted_result_ids2=array_msort($sorted_result_ids,array('step'=>SORT_ASC,'points'=>SORT_DESC,'points_marqués'=>SORT_DESC,'ippons_marqués'=>SORT_DESC));
	...
	$i=1;
		foreach($sorted_result_ids2 as $s2){
			if($s2["step"]=="Poule A" |$s2["step"]=="Poule B" |$s2["step"]=="Poule C"| $s2["step"]=="Poule D" ){
				if($step_prec && ($step_prec!=$s2["step"] )){
					$i=1;
				}
